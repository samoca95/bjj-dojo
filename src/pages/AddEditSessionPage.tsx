...
export default function AddEditSessionPage() {
  ...
  type PickerEntry =
    | { kind: 'technique'; item: Technique }
    | { kind: 'flow'; item: Flow }

  const combinedPickerResults = useMemo((): PickerEntry[] | null => {
    if (!pickerSearch.trim()) return null
    const nameById = (id: number) => techniqueNameById.get(id) ?? ''
    const noteMap = sessionTechniqueNotes ?? new Map<number, string[]>()
    const techEntries = filteredTechniques.map((t) => ({
    ...
    const flowEntries = filteredPickerFlows.map((f) => ({
      kind: 'flow' as const,
      item: f,
      score: flowScore(f, nameById, pickerSearch),
    }))
    return [...techEntries, ...flowEntries]
      .sort((a, b) => b.score - a.score)
      .map(({ kind, item }) => ({ kind, item })) as PickerEntry[]
  }, [
    pickerSearch,
    filteredTechniques,
    filteredPickerFlows,
