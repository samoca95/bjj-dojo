...
export default function SessionTypeIconsPage() {
  ...
  const [sessionTypeIcons, setSessionTypeIcons] = useState<SessionTypeIconsMap>(
    getSessionTypeIcons(),
  )
    ...
    <div className="min-h-full bg-zinc-950">
      ...
      <div className="px-4 pb-6 space-y-3">
        {SESSION_TYPES.map((sessionType) => {
          const isDefault =
            sessionTypeIcons[sessionType] === SESSION_TYPE_ICONS[sessionType]
          return (
            <div
              key={sessionType}
            ...
            >
              <div className="w-11 h-11 rounded-xl bg-zinc-800 flex items-center justify-center shrink-0">
                <CategoryIcon
                  value={sessionTypeIcons[sessionType]}
                  size={20}
                  className="text-gold"
                />
              </div>
              ...
              {!isDefault && (
                <button
                  onClick={() => {
                    const next = {
                      ...sessionTypeIcons,
                      [sessionType]: SESSION_TYPE_ICONS[sessionType],
