...
function SessionCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={`w-10 h-10 rounded-xl shrink-0 flex items-center justify-center ${SESSION_TYPE_COLORS[session.sessionType].replace('text-', 'text-').replace('bg-', 'bg-')}`}
      >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-xs font-bold px-1.5 py-0.5 rounded ${SESSION_TYPE_COLORS[session.sessionType]}`}
          >
            {sessionTypeLabel(
              session.sessionType,
        ...
        <div className="flex items-center gap-3 mt-0.5">
          ...
          {clubName && (
            <span className="text-xs text-zinc-500 truncate">{clubName}</span>
          )}
...
export default function SessionsPage() {
  ...
  const sessionMeta = useLiveQuery(
    async () => {
      ...
      const techniqueNameById = new Map(
        techniques.map((technique) => [technique.id, technique.name]),
      )
      ...
      for (const st of sts) {
        ...
        const techniqueSearchText =
          searchableTechniqueTextById.get(st.techniqueId) ?? name.toLowerCase()
      ...
      const tapStatsBySessionId = new Map<
        number,
        { given: number; received: number }
      >()
      for (const tap of taps) {
        const bucket = tapStatsBySessionId.get(tap.sessionId) ?? {
          given: 0,
          received: 0,
        }
  ...
  const cutoff =
    daysFilter === 'all' ? null : Date.now() - daysFilter * 24 * 60 * 60 * 1000
  ...
  const visibleSessions = (sessions ?? []).filter((session) => {
    ...
    if (searchTokens.length > 0) {
      ...
      const clubName =
        session.clubId !== null && session.clubId !== undefined
          ? clubMap.get(session.clubId)
          : ''
      const tapStats = sessionMeta.tapStatsBySessionId?.get(sessionId) ?? {
        given: 0,
        received: 0,
      }
      const searchable = [
  ...
  const sortedVisibleSessions = [...visibleSessions].sort((a, b) => {
    const tapsA = sessionMeta.tapStatsBySessionId.get(a.id ?? -1) ?? {
      given: 0,
      received: 0,
    }
    const tapsB = sessionMeta.tapStatsBySessionId.get(b.id ?? -1) ?? {
      given: 0,
      received: 0,
    }
    if (sortBy === 'date_asc') return a.date - b.date
    if (sortBy === 'duration')
      return b.durationMinutes - a.durationMinutes || b.date - a.date
    if (sortBy === 'taps_total') {
      const totalDelta =
        tapsB.given + tapsB.received - (tapsA.given + tapsA.received)
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <option value="date_desc">
                {language === 'es' ? 'Fecha (más reciente)' : 'Date (newest)'}
              </option>
              <option value="date_asc">
                {language === 'es' ? 'Fecha (más antigua)' : 'Date (oldest)'}
              </option>
              <option value="duration">
                {language === 'es' ? 'Duración' : 'Duration'}
              </option>
              <option value="taps_total">
                {language === 'es' ? 'Taps (total)' : 'Taps (total)'}
              </option>
              <option value="taps_given">
                {language === 'es' ? 'Taps dados' : 'Taps (given)'}
              </option>
              <option value="taps_received">
                {language === 'es' ? 'Taps recibidos' : 'Taps (received)'}
              </option>
            </select>
            <button
          ...
          <div className="space-y-3">
            {sortedVisibleSessions.map((s) => (
              <SessionCard
                ...
                language={language}
                tapStats={
                  sessionMeta.tapStatsBySessionId.get(s.id ?? -1) ?? {
                    given: 0,
                    received: 0,
                  }
                }
                techniqueNames={
                  sessionMeta.techniqueNamesBySessionId.get(s.id ?? -1) ?? []
                }
                clubName={
                  s.clubId !== null && s.clubId !== undefined
                    ? clubMap.get(s.clubId)
                    : undefined
                }
                onClick={() => {
