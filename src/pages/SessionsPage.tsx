      ...
      <div className="px-4 pb-24 space-y-4">
        ...
        <div className="bg-zinc-900 rounded-2xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <h2 className="text-xs font-semibold tracking-widest text-gold">{t('WEEKLY GOAL')}</h2>
          ...
          <p className="text-xs text-zinc-500">
            {t('Current streak')}: {streak} {language === 'es' ? `día${streak === 1 ? '' : 's'}` : `day${streak === 1 ? '' : 's'}`}
          </p>
        </div>

        <div className="bg-zinc-900 rounded-2xl p-4">
          <h2 className="text-xs font-semibold tracking-widest text-gold mb-3">{t('TREND (LAST 8 SESSIONS)')}</h2>
          {trendData.length === 0 ? (
            <p className="text-xs text-zinc-500">{t('No session data in the selected range.')}</p>
          ) : (
