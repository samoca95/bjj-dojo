...
export default function SessionDetailPage() {
  ...
  const techniqueEntries = useLiveQuery(
    async () => {
      ...
      return techs.map((tech) => ({
        technique: tech,
        notes: notesMap.get(tech.id),
      }))
    },
    [id],
    [] as { technique: Technique; notes: string | undefined }[],
  )

  const tapData = useLiveQuery(
    async () => {
      if (!id)
        return { taps: [] as SessionTap[], techMap: new Map<number, string>() }
      const taps = await db.sessionTaps
        .where('sessionId')
        .equals(Number(id))
        .toArray()
      if (taps.length === 0)
  ...
  const shareData = {
    session,
    clubName: club?.name,
    techniques: (techniqueEntries ?? []).map(({ technique, notes }) => ({
      technique,
      notes,
    })),
    ...
    <div className="min-h-full bg-zinc-950">
      ...
      <div className="sticky top-0 bg-zinc-950/90 backdrop-blur-sm px-4 pt-12 pb-4 z-10 flex items-center gap-3">
        ...
        <button
          ...
          className="p-2 text-red-500 active:text-red-400"
        >
      ...
      <div className="px-4 space-y-4 pb-6">
        ...
        {showSharePrompt && (
          <div className="flex items-center gap-3 bg-gold/10 border border-gold/30 rounded-xl px-4 py-3">
            ...
            <span className="flex-1 text-sm text-zinc-100">
              {SHARE_PROMPT_LABELS[language].title}
            </span>
            <button
              onClick={() => {
        ...
        <div
          className={`inline-flex items-center gap-2 text-sm font-bold px-3 py-1.5 rounded-lg ${SESSION_TYPE_COLORS[session.sessionType]}`}
        >
          <CategoryIcon
            value={sessionTypeIcons[session.sessionType]}
            size={16}
        ...
        <div className="grid grid-cols-2 gap-3">
          <InfoCard
            label={t('Duration')}
            value={`${session.durationMinutes} ${t('min')}`}
          />
          <div className="bg-zinc-900 rounded-xl p-4">
