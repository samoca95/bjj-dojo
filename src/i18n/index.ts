...
const ES_TECHNIQUE_CONTENT: Record<number, { description: string; cues: string[] }> = {
  // Guards (1xx)
  101: {
...
export function getCategoryName(category: Category, language: AppLanguage): string {
  ...
  return ES_CATEGORY_CONTENT[category.id]?.name ?? category.name
}
...
export function getCategoryDescription(category: Category, language: AppLanguage): string {
  ...
  return ES_CATEGORY_CONTENT[category.id]?.description ?? category.description
}

export function getTechniqueDescription(technique: Technique, language: AppLanguage): string {
  if (language !== 'es' || technique.isCustom) return technique.description
  return ES_TECHNIQUE_CONTENT[technique.id]?.description ?? technique.description
}

export function getTechniqueCues(technique: Technique, language: AppLanguage): string[] {
  if (language !== 'es' || technique.isCustom) return technique.cues ?? []
  return ES_TECHNIQUE_CONTENT[technique.id]?.cues ?? technique.cues ?? []
}

export function translateCategoryForExport(category: Category, language: AppLanguage): Category {
  if (language !== 'es') return category
  const es = ES_CATEGORY_CONTENT[category.id]
  if (!es) return category
  return { ...category, name: es.name, description: es.description }
}

export function translateTechniqueForExport(technique: Technique, language: AppLanguage): Technique {
  if (language !== 'es' || technique.isCustom) return technique
  const es = ES_TECHNIQUE_CONTENT[technique.id]
  if (!es) return technique
  return { ...technique, description: es.description, cues: es.cues }
}
