...
function readTelemetry(): TelemetryEntry[] {
  ...
  try {
    ...
    return parsed.filter(
      (item) => item && typeof item.event === 'string',
    ) as TelemetryEntry[]
  } catch {
    return []
  }
...
function writeTelemetry(entries: TelemetryEntry[]) {
  ...
  try {
    window.localStorage.setItem(
      ...
      JSON.stringify(entries.slice(-MAX_TELEMETRY_ENTRIES)),
    )
  } catch {
...
export function logTelemetry(
  ...
  if (level === 'error') {
    ...
    // eslint-disable-next-line no-console -- telemetry intentionally emits info-level logs
...
export const telemetry = {
  info: (event: string, context?: unknown) =>
    logTelemetry('info', event, context),
  warn: (event: string, context?: unknown) =>
    logTelemetry('warn', event, context),
