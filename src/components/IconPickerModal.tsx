...
export default function IconPickerModal({
  ...
  const suggestedIcons = useMemo(
    () =>
      CATEGORY_ICON_OPTIONS.filter((icon) =>
        SUGGESTED_ICON_IDS.includes(icon.id),
      ),
    [],
  )
  ...
  const filteredIconIds = useMemo(() => {
    ...
    return ALL_ICON_IDS.filter(
      (id) =>
        id.includes(query) || iconIdToLabel(id).toLowerCase().includes(query),
    )
  }, [search])
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-[60] flex items-end pb-[calc(4rem+env(safe-area-inset-bottom))]">
      <div className="bg-zinc-900 w-full rounded-t-3xl max-h-[calc(100dvh-4rem-env(safe-area-inset-bottom)-0.5rem)] flex flex-col">
          ...
          <>
            ...
            <div className="px-4 pb-4 overflow-y-auto min-h-0 flex-1 space-y-5">
              ...
              <div>
                {showSuggested && (
                  <div className="text-xs text-zinc-500 font-semibold mb-2">
                    {tr('All Icons')} ({ALL_ICON_IDS.length})
                  </div>
                )}
                <div className="grid grid-cols-5 gap-2">
