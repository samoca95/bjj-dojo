...
export const ICON_MAP: Record<string, CategoryIconOption> = Object.fromEntries(
  CATEGORY_ICON_OPTIONS.map((icon) => [icon.id, icon]),
)
...
export function resolveCategoryIcon(icon?: string, fallbackId?: number) {
  if (icon) return icon
  if (fallbackId && DEFAULT_CATEGORY_ICONS[fallbackId])
    return DEFAULT_CATEGORY_ICONS[fallbackId]
  return undefined
}
